...
def _backend_settings(
    ...
    *,
    ...
    learning_store_path: str = "data/test-learned.json",
    learning_min_similarity: float = 0.70,
    enable_internet_learning: bool = False,
    internet_cache_path: str = "data/test-internet-cache.json",
    internet_timeout_seconds: int = 8,
    internet_max_summary_chars: int = 700,
    internet_cache_ttl_days: int = 14,
    internet_allowed_domains: tuple[str, ...] = ("en.wikipedia.org", "wikipedia.org"),
    internet_source_providers: tuple[str, ...] = ("wikipedia",),
    internet_max_sources: int = 1,
) -> BackendSettings:
...
class TestBot(unittest.TestCase):
    ...
    def test_openai_backend_falls_back_without_token(self):
        backend = create_backend(_backend_settings("openai"))
    ...
    def test_openai_backend_raises_without_token_when_fallback_disabled(self):
        with self.assertRaises(ValueError):
            create_backend(_backend_settings("openai", allow_backend_fallback=False))
